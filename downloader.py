...
def download(leftover, pic_name, pic_host):
...
def get_one(thread_obj):
    ...
    resp = requests.get(url+'1&type=3', headers=header).content.decode('utf-8')
    total = int(json.loads(resp)['data']['total'])
    total = ceil(total/32)
    for num in range(1, total+1):
    ...
    for pic_name, pic_host in pic_dict.items():
        download(leftover, pic_name, pic_host)
        leftover = leftover - 1
...
def get_all_pic(thread_obj):
    ...
    for key, value in thread_obj.album_dict.items():
        ...
        if key == "微博配图":
            resp = requests.get(url+"1&type=3", headers=header)
        elif key == "头像相册":
            resp = requests.get(url+"1&type=1", headers=header)
        else:
            resp = requests.get(url+"1&type=1", headers=header)
        total = int(json.loads(resp.content.decode('utf-8'))['data']['total'])
        total = ceil(total/32)
        #piclist = []
        ...
        for num in range(1, total+1):
            if key == "微博配图":
                resp = requests.get(url+str(num)+'&type=3', headers=header)
            elif key == "头像相册":
                resp = requests.get(url+str(num)+'&type=18', headers=header)
            else:
                resp = requests.get(url+str(num)+'&type=1', headers=header)
