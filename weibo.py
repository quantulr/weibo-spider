...
def get_username(uid):
	...
	user_fold_name = json.loads(html.content.decode('utf-8'))['data']['userInfo']['screen_name']
...
def get_album_dict(uid):
	...
	for num in range(1,total+1):
		url = 'http://photo.weibo.com/albums/get_all?uid='+uid+'&page='+str(num)+'&count=20'
		html = requests.get(url, headers=header)
...
def get_one(thread_obj):
	...
	pic_dict = {}
	url = 'http://photo.weibo.com/photos/get_all?uid='+thread_obj.uid+'&album_id='+thread_obj.album_dict['微博配图']+'&count=32&page='
	...
	for num in range(1,total+1):
		...
		thread_obj.statusSignal.emit('第'+str(num)+'/'+str(total)+'页共有'+str(len(temp))+'张照片！')
		for x in temp:
	...
	for key, value in pic_dict.items():
		...
		thread_obj.statusSignal.emit("还剩"+str(leftover)+"/"+str(pic_total)+"项。")	
...
def get_all_pic(thread_obj):
	...
	for key, value in thread_obj.album_dict.items():
		...
		url = 'http://photo.weibo.com/photos/get_all?uid='+thread_obj.uid+'&album_id='+value+'&count=32&page='	
		if key == "微博配图":
			html = requests.get(url+"1&type=3", headers=header)
		elif key == "头像相册":
